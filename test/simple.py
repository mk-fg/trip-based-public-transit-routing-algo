...
class SimpleTestCase(unittest.TestCase):
	...
	def run_test(self):
		...
		goal = common.struct_from_val(self.test_data.goal, common.TestGoal)
...
class SimpleGraphTests(unittest.TestSuite):
	...
	def __init__(self):
		...
		tests, test_data = list(), common.load_test_data(
			path_file.parent, path_file.stem, 'journey-planner-csa' )
		tb, types = common.tb, common.tb.t.public
		...
		for test_name, test in test_data.items():
			...
			for trip_id, trip_data in test.timetable.items():
				...
				for stopidx, ts in enumerate(trip_data):
					stop_id, dts_arr, dts_dep = common.struct_from_val(ts, TestTripStop, as_tuple=True)
					if not dts_arr or dts_arr == 'x': dts_arr = dts_dep
					if not dts_dep or dts_dep == 'x': dts_dep = dts_arr
					dts_arr, dts_dep = map(common.dts_parse, [dts_arr, dts_dep])
			...
			router = tb.engine.TBRoutingEngine(
				timetable, timer_func=common.gtfs_cli.calc_timer )
			checks = common.GraphAssertions(router.graph)
